...
import { Holiday, DataCollectionResult, CacheEntry } from '../types';
...
export class HolidayDataCollector {
  private apiClient: HolidayApiClient;
  private dataDir: string;
  private cacheDir: string;
  private cache: Map<string, CacheEntry<any>> = new Map();
  private cacheTTL: number = 24 * 60 * 60 * 1000; // 24시간

  constructor(apiClient: HolidayApiClient, dataDir: string = 'data') {
  ...
  async collectHolidayData(
    ...
    try {
      // 캐시 확인
      if (useCache) {
        const cachedData = await this.getCachedData<Holiday[]>(cacheKey);
        if (cachedData) {
   ...
   */
  async collectMultipleCountries(
    countryCodes: string[], 
    year: number
   ...
   */
  private validateAndNormalizeHolidays(
    holidays: Holiday[], 
    countryCode: string, 
    year: number
  ): Holiday[] {
   ...
   */
  private async saveHolidaysToFile(
    countryCode: string, 
    year: number, 
    holidays: Holiday[]
  ): Promise<void> {
   ...
   */
  private async loadHolidaysFromFile(countryCode: string, year: number): Promise<Holiday[]> {
    try {
      const fileName = `${countryCode.toLowerCase()}-${year}.json`;
      const filePath = path.join(this.dataDir, 'holidays', fileName);
   ...
   */
  async hasData(countryCode: string, year: number): Promise<boolean> {
    try {
      const fileName = `${countryCode.toLowerCase()}-${year}.json`;
      const filePath = path.join(this.dataDir, 'holidays', fileName);
 ...
 */
export function createHolidayDataCollector(apiClient: HolidayApiClient): HolidayDataCollector {
